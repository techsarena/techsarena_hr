...
function AssignModal({ open, onClose, alert, onDone }) {
  ...
  return (
    <Modal
      ...
      subtitle="Submits a standard HRMS Leave Policy Assignment"
      footer={
        <>
...
function DemoCard({ onDone }) {
  ...
  return (
    <>
      <Card title="Demo &amp; testing" subtitle="Provisions a demo workforce for evaluating the app">
        <p className="muted small">
        ...
        <div style={{ marginTop: 'var(--space-4)' }}>
          <Button variant="ghost" onClick={() => setConfirm(true)}>Seed demo data</Button>
        </div>
      </Card>
  ...
  return (
    <Drawer open onClose={onClose} title={item.title} subtitle={item.description}>
      {(item.details || []).length === 0 ? (
        <EmptyState title="No detail returned" body="This card has no further breakdown on this site." icon="◷" />
      ) : (
        <Card flush>
  ...
  return (
    <div className="stack">
      <div className="page-head">
        ...
        <p className="page-head__sub">The HRMS configuration behind this workspace</p>
      </div>
          ...
          return (
            <>
              <div className="grid grid--3">
                <div className="card"><Stat label="Organisation" value={data.organisation_name || '—'} /></div>
                <div className="card"><Stat label="Active staff" value={data.staff_count ?? '—'} /></div>
                <div className="card">
                  <Stat
                    label="Your access"
              ...
              {unassigned > 0 && (
                <Card
                  title="Employees without a leave policy"
                  subtitle={`${unassigned} employee${unassigned === 1 ? '' : 's'} have no active assignment`}
                  action={data.can_edit && <Button variant="indigo" size="sm" onClick={() => setAssignOpen(true)}>Assign</Button>}
                >
              ...
              {(data.health || []).length > 0 && (
                <Card title="Configuration coverage" subtitle="How much of the workforce each setup step covers">
              ...
              <div className="grid grid--2">
                <Card title="Recent changes">
                  {(data.recent_changes || []).length === 0 ? (
                    <EmptyState title="No recent changes" icon="◷" />
                  ) : (
                    <div className="stack">
                ...
                <Card title="Who can edit setup">
                  {(data.editors || []).length === 0 ? (
                    <EmptyState title="No editors listed" icon="◷" />
                  ) : (
                    <div className="stack">
              ...
              <Card title="Manage in the desk" subtitle="Record-level editing keeps Frappe's own validation and audit trail">
